...
class StatsRepositoryImpl @Inject constructor() : StatsRepository {
    override fun calculateBinomialPmf(n:Int,p:Double,k:Int):Double=BinomialCalculator.pmf(n,p,k)
    override fun calculateBinomialCdfLower(n:Int,p:Double,k:Int):Double=BinomialCalculator.cdfLower(n,p,k)
    override fun calculateBinomialCdfUpper(n:Int,p:Double,k:Int):Double=BinomialCalculator.cdfUpper(n,p,k)
    override fun calculatePoissonPmf(lambda:Double,k:Int):Double=PoissonCalculator.pmf(lambda, k)
    override fun calculatePoissonCdfLower(lambda:Double,k:Int):Double=PoissonCalculator.cdfLower(lambda, k)
    override fun calculatePoissonCdfUpper(lambda:Double,k:Int):Double=PoissonCalculator.cdfUpper(lambda, k)
    override fun calculateNormalCdf(z:Double):Double=NormalCalculator.cdf(z)
